...
function(get_spparks_version version_header variable)
    file(STRINGS ${version_header} line REGEX SPPARKS_VERSION)
    set(MONTHS x Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec)
    string(REGEX REPLACE "#define SPPARKS_VERSION \"([0-9]+) ([A-Za-z]+) ([0-9]+)\"" "\\1" day "${line}")
    string(REGEX REPLACE "#define SPPARKS_VERSION \"([0-9]+) ([A-Za-z]+) ([0-9]+)\"" "\\2" month "${line}")
    string(REGEX REPLACE "#define SPPARKS_VERSION \"([0-9]+) ([A-Za-z]+) ([0-9]+)\"" "\\3" year "${line}")
    ...
    #   set(month "0${month}")
    #endif()
    file(STRINGS ${version_header} line REGEX SPPARKS_UPDATE)
    string(REGEX REPLACE "#define SPPARKS_UPDATE \"Update ([0-9]+)\"" "\\1" tweak "${line}")
    if (line MATCHES "#define SPPARKS_UPDATE \"(Maintenance|Development)\"")
      set(tweak "99")
    endif()
    if(NOT tweak)
      set(tweak "0")
...
function(check_for_autogen_files source_dir)
    ...
    list(APPEND SRC_AUTOGEN_FILES ${SRC_AUTOGEN_PACKAGES} ${source_dir}/spkinstalledpkgs.h ${source_dir}/spkgitversion.h)
...
macro(pkg_depends PKG1 PKG2)
  if(DEFINED BUILD_${PKG2})
    if(PKG_${PKG1} AND NOT BUILD_${PKG2})
      message(FATAL_ERROR "The ${PKG1} package needs SPPARKS to be built with -D BUILD_${PKG2}=ON")
    endif()
  ...
  if(DOWNLOAD_POTENTIALS)
    if(EXISTS "${pkgfolder}/potentials.txt")
      ...
      foreach(line ${linelist})
        ...
        if(EXISTS "${SPPARKS_POTENTIALS_DIR}/${pot}")
          file(MD5 "${SPPARKS_POTENTIALS_DIR}/${pot}" oldsum)
        endif()
        if(NOT sum STREQUAL oldsum)
          message(STATUS "Downloading external potential ${pot} from ${SPPARKS_POTENTIALS_URL}")
          string(RANDOM LENGTH 10 TMP_EXT)
          file(DOWNLOAD "${SPPARKS_POTENTIALS_URL}/${pot}.${sum}" "${CMAKE_BINARY_DIR}/${pot}.${TMP_EXT}"
            EXPECTED_HASH MD5=${sum} SHOW_PROGRESS)
          file(COPY "${CMAKE_BINARY_DIR}/${pot}.${TMP_EXT}" DESTINATION "${SPPARKS_POTENTIALS_DIR}")
          file(RENAME "${SPPARKS_POTENTIALS_DIR}/${pot}.${TMP_EXT}" "${SPPARKS_POTENTIALS_DIR}/${pot}")
        endif()
      endforeach()
    endif()
  endif()
...
function(GetFallbackURL input output)
  ...
  string(REGEX REPLACE "^https://.*/([^/]+gz)" "${SPPARKS_THIRDPARTY_URL}/${libname}-\\1" newurl "${${input}}")
