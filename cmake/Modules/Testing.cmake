...
if(ENABLE_TESTING)
  ...
  file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/spparks.supp "\n")
  file(GLOB VALGRIND_SUPPRESSION_FILES CONFIGURE_DEPENDS ${SPPARKS_TOOLS_DIR}/valgrind/[^.]*.supp)
  foreach(SUPP ${VALGRIND_SUPPRESSION_FILES})
    file(READ ${SUPP} SUPPRESSIONS)
    file(APPEND ${CMAKE_CURRENT_BINARY_DIR}/spparks.supp "${SUPPRESSIONS}")
  endforeach()
  set(VALGRIND_DEFAULT_OPTIONS "--leak-check=full --show-leak-kinds=all --track-origins=yes --suppressions=${CMAKE_BINARY_DIR}/spparks.supp")
  ...
  if((CMAKE_SYSTEM_NAME STREQUAL "Linux")
    ...
    if(((CMAKE_LINUX_DISTRO STREQUAL "Ubuntu") AND (CMAKE_DISTRO_VERSION VERSION_GREATER_EQUAL 22.04))
      ...
      if(NOT "${CMAKE_CUSTOM_LINKER}" STREQUAL "default")
        target_link_options(spparks PUBLIC -fuse-ld=${CMAKE_CUSTOM_LINKER})
      endif()
      if(HAVE_LD_PATH_FLAG)
        if("${CMAKE_CUSTOM_LINKER}" STREQUAL "mold")
          target_link_options(spparks PUBLIC --ld-path=${HAVE_MOLD_LINKER_BIN})
        elseif("${CMAKE_CUSTOM_LINKER}" STREQUAL "lld")
          target_link_options(spparks PUBLIC --ld-path=${HAVE_LLD_LINKER_BIN})
        elseif("${CMAKE_CUSTOM_LINKER}" STREQUAL "gold")
          target_link_options(spparks PUBLIC --ld-path=${HAVE_GOLD_LINKER_BIN})
        elseif("${CMAKE_CUSTOM_LINKER}" STREQUAL "bfd")
          target_link_options(spparks PUBLIC --ld-path=${HAVE_BFD_LINKER_BIN})
        endif()
      endif()
    endif()
  endif()
  ...
  enable_testing()
  get_filename_component(SPPARKS_UNITTEST_DIR ${SPPARKS_SOURCE_DIR}/../unittest ABSOLUTE)
  get_filename_component(SPPARKS_UNITTEST_BIN ${CMAKE_BINARY_DIR}/unittest ABSOLUTE)
  add_subdirectory(${SPPARKS_UNITTEST_DIR} ${SPPARKS_UNITTEST_BIN})
endif()
...
if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
  ...
  if(ENABLE_COVERAGE)
    target_compile_options(spparks PUBLIC --coverage)
    target_link_options(spparks PUBLIC --coverage)
  endif()
endif()
...
if(NOT ENABLE_SANITIZER STREQUAL "none")
  if((${CMAKE_CXX_COMPILER_ID} STREQUAL "GNU") OR (${CMAKE_CXX_COMPILER_ID} STREQUAL "Clang"))
    target_compile_options(spparks PUBLIC -fsanitize=${ENABLE_SANITIZER})
    target_link_options(spparks PUBLIC -fsanitize=${ENABLE_SANITIZER})
  else()
